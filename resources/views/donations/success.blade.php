...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    ...
    <div class="bg-white rounded-lg shadow-lg p-8 mb-6">
        ...
        <div class="bg-blue-50 border-2 border-blue-200 rounded-lg p-6 mb-6 text-center">
            ...
            <p class="text-3xl font-bold text-blue-600 mb-2">{{ $donation->tracking_code }}</p>
        ...
        <div class="space-y-4 mb-6">
            <div class="flex justify-between pb-3 border-b">
                ...
                <span class="font-bold text-xl text-gray-800">₱{{ number_format($donation->amount, 2) }}</span>
            </div>
            ...
            <div class="flex justify-between pb-3 border-b">
                ...
                <span class="font-semibold text-gray-800">{{ $donation->created_at->format('M d, Y \a\t h:i A') }}</span>
            </div>

            <div class="flex justify-between">
                <span class="text-gray-600">Status:</span>
                <span class="px-3 py-1 bg-{{ $donation->status_color }}-100 text-{{ $donation->status_color }}-800 font-semibold rounded-full text-sm">
    ...
    <div class="flex flex-col sm:flex-row gap-4">
        <a href="{{ route('home') }}" class="flex-1 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors text-center">
            Return to Map
        </a>
        <a href="{{ route('donation.track') }}" onclick="event.preventDefault(); document.getElementById('track-form').submit();" class="flex-1 px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold rounded-lg transition-colors text-center">
            Track This Donation
        </a>
    </div>

    <!-- Hidden form for tracking -->
    <form id="track-form" action="{{ route('donation.track') }}" method="POST" style="display: none;">
        @csrf
        <input type="hidden" name="tracking_code" value="{{ $donation->tracking_code }}">
    </form>
