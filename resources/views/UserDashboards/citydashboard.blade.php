...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/city/layout.css') }}">
    <link rel="stylesheet" href="{{ asset('css/city/map.css') }}">
    <link rel="stylesheet" href="{{ asset('css/city/modals.css') }}">

    <style>
        {{-- Remaining inline styles that need conversion --}}
    </style>
...
<template id="chat-box-template">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Donation Status</th>
                            ...
                            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700">Needs Help?</th>
    ...
    <script>
        ...
        let cityMap = null;
        let loadedTabs = { map: false, resources: false, barangays: false };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (modal) {
        ...
        async function loadBarangaysComparison() {
            ...
            try {
                ...
                tbody.innerHTML = barangays.map(b => {
                    ...
                    const donationStatus = b.donation_status || 'pending';
                    const needsHelp = b.needs_help || false;

                    return `
                        <tr class="border-b hover:bg-gray-50">
                            <td class="px-4 py-3 font-medium">${escapeHtml(b.name)}</td>
                            <td class="px-4 py-3">
                                <span class="px-2 py-1 text-xs rounded font-semibold ${getStatusBadge(donationStatus)}">
                                    ${getStatusLabel(donationStatus)}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ${needsHelp ?
                                        '<span class="px-2 py-1 bg-red-100 text-red-700 text-xs rounded font-semibold"><i class="fas fa-exclamation-circle mr-1"></i>Yes</span>' :
                                        '<span class="px-2 py-1 bg-green-100 text-green-700 text-xs rounded font-semibold"><i class="fas fa-check-circle mr-1"></i>No</span>'
                                    }
                                </div>
                            </td>
                            <td class="px-4 py-3">
        ...
        function getStatusBadge(status) {
            const badges = {
                // New donation status system
                'pending': 'bg-red-100 text-red-700',
                'in_progress': 'bg-orange-100 text-orange-700',
                'completed': 'bg-green-100 text-green-700',
                // Legacy disaster status (for backwards compatibility)
