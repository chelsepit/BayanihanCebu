...
const CONTRACT_ABI = [
    ...
    },
    {
        "anonymous": false,
        "inputs": [
            {"indexed": false, "internalType": "string", "name": "trackingCode", "type": "string"},
            {"indexed": false, "internalType": "uint256", "name": "amount", "type": "uint256"},
            {"indexed": false, "internalType": "string", "name": "barangayId", "type": "string"},
            {"indexed": false, "internalType": "enum DonationRecorder.DonationType", "name": "donationType", "type": "uint8"},
    ...
    },
    {
        "inputs": [
            ...
            {"internalType": "enum DonationRecorder.DonationType", "name": "donationType", "type": "uint8"}
        ],
        "name": "recordDonation",
        "outputs": [],
        "stateMutability": "nonpayable",
...
async function recordDonation() {
    try {
        ...
        if (args.length < 4) {
            ...
            console.error('Usage: node recordDonation.js <trackingCode> <amount> <barangayId> <donationType>');
            console.error('Example: node recordDonation.js "DON-ABC123" "1000" "BRG001" "monetary"');
            process.exit(1);
        }
        ...
        const privateKey = process.env.PRIVATE_KEY || process.env.LISK_PRIVATE_KEY || process.env.METAMASK_PRIVATE_KEY;
        if (!privateKey) {
            throw new Error('PRIVATE_KEY (or LISK_PRIVATE_KEY or METAMASK_PRIVATE_KEY) not set in .env file');
        }
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                provider = new ethers.providers.JsonRpcProvider(process.env.LISK_RPC_URL);
                const blockNumber = await provider.getBlockNumber();
                console.log('✓ Connected to Lisk Sepolia (Block:', blockNumber + ')');
                connected = true;
                break;
            } catch (error) {
                console.log(`⚠️  Connection attempt ${attempt + 1}/${maxRetries} failed:`, error.message);
                ...
                if (attempt < maxRetries - 1) {
                    ...
                    throw new Error('Failed to connect to Lisk RPC after ' + maxRetries + ' attempts: ' + error.message);
                }
            }
        }
        ...
        if (balance.isZero()) {
            ...
            console.error('Get test ETH from: https://sepolia-faucet.lisk.com/');
        ...
        try {
            ...
            console.log('✓ Wallet is authorized', isOwner ? '(Owner)' : '(Admin)');

            // Check if contract is paused
            const isPaused = await contract.paused();
            if (isPaused) {
                console.error('❌ Contract is paused');
                console.error('Solution: Call togglePause() in Remix to unpause');
            ...
            if (error.message.includes('not authorized') || error.message.includes('paused')) {
                throw error;
            }
        ...
        for (let txAttempt = 0; txAttempt < maxTxRetries; txAttempt++) {
            try {
                ...
                console.log(`⚠️  Transaction attempt ${txAttempt + 1}/${maxTxRetries} failed:`, error.message);
        ...
        if (error.code === 'CALL_EXCEPTION') {
            ...
            console.error('1. Wallet is not an admin - Add wallet as admin in Remix');
            console.error('2. Contract is paused - Call togglePause() in Remix');
